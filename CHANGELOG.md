...
- **French end-to-end** �?the site is now trilingual (en/es/fr). The content
  ...
  proves French output across both content files and the TS data layer �?  292 tests total. Edge-cache warming now includes `/fr`.
- **AI index deploy pipeline** (`.github/workflows/ai-index.yml`) �?after every
  ...
  the three Vectorize indexes (`stavalk-knowledge`, `-staging`, `-prod`,
...
- **AI sales assistant** (`src/features/ai/`) �?a floating RAG chat widget on the
  ...
  products/news/technology/case-studies/pages, and every site FAQ as its
  ...
  cron via stable-hash upserts. i18n under `bench.aiChat.*` (en/es/fr).
- **Site search** �?three surfaces over one index: a header search dialog fed by
  ...
  pages, knowledge hub, projects, product series, products/news/technology/
...
- **AEO/GEO surfaces brought current** �?`entity.json` now serves a
  Stavalk-specific `description` (plant scale, MOQ tiers, lead times) and a
  ...
  current tiered MOQ (1�? samples / 20�?0 trial / 90�?00+ volume) en+es.
- **SEO meta spec enforced** �?news titles/excerpts, product meta (26 files) and
  YAML page meta (34 files) trimmed to `title �?70` / `description 80�?70` chars,
...
- **Whole-site audit fixes** �?  - `/about/afarer` (en+es) was rendering empty: registry slug `about-Afarer`
  ...
  - Stale standalone claims: `numberOfEmployees` 200+ �?350+ in brand JSON-LD
    ...
    old "stavalk" brand-only framing, case-study hub intro (en/es) named the old brand as the seller — all brought to the current Stavalk framing; removed dead `trackEngage` and the committed `cf-inspect.log`.
...
- **Deploy pipeline** �?CDN cache purge after every release (token now carries
  `Zone �?Cache Purge`), edge-cache warming and image backfill steps.
...
[Unreleased]: https://github.com/Stavalk/web/compare/v1.1.0...main
[1.1.0]: https://github.com/Stavalk/web/releases/tag/v1.1.0
[1.0.0]: https://github.com/Stavalk/web/releases/tag/v1.0.0