...
**Meta length spec** (enforced on news/products/yaml pages): `title �?70` chars, `description 80�?70` chars, kept in sync across locales �?keeps SERP snippets clean.
...
The five legacy landing routes (`custom-sup-manufacturing`, `private-label-sup`, `sup-for-resorts`, `sup-for-clubs`, `sup-startup-brands`) are stubs whose loaders throw `redirect({ href: localizePath(locale, target), statusCode: 301 })`. Broader URL policy (stavalk-era dups, removed pages, trailing slashes, retired `/zh`) lives in the **edge gate** (`src/features/seo/edge-gate.ts`) so it runs before SSR:
...
| Layer | Piece |
|-------|-------|
| **matchFaq** | Token-overlap scoring against `getSiteFaqs(locale)`: splits question + FAQ question into word tokens, computes Jaccard-like overlap (�? significant words, �?5% overlap for English; CJK: character-level tokenization + cross-language keyword expansion, min score 0.30 vs 0.55). Returns the best-matching FAQ answer. |
