# Stavalk — Technical Documentation

> Last updated: 2026-09-09
> Project path: `E:\github\Stavalk`
> Production: https://Stavalk.com (Cloudflare Workers, `Stavalk-production`)
> Stack: TanStack Start (React 19) + Cloudflare Workers + D1 (Drizzle ORM) + KV + R2 + better-auth + Resend + Orama (search) + Fumadocs (docs)
> > Media: All large assets (videos, PDFs, quality photos, product photos) migrated to Cloudflare R2 bucket `Stavalk-files-prod`, served via CDN `assets.Stavalk.com/site/*`; `public/assets/*` directories added to `.gitignore`; upload script `scripts/upload-site-assets.mjs` supports `--prefix <prefix>` for multi-site key isolation.
...
|----------|--------|--------|
| `/sitemap.xml` | `src/features/seo/seo.ts` (`PUBLIC_PATHS` × locales, hreflang alternates) + docs pages + public paths (registry + products/news/technology/case-studies/guides) | XML; en/es/fr entries with hreflang alternates across the trilingual page set |
...
| `/llms.txt` | `src/features/docs/llm.ts` (docs index) + `src/features/site/llm.ts` (products + **solution pages** + index) | Markdown index for LLMs |
| `/llms-full.txt` | same, concatenated plain Markdown | full corpus (catalog, solutions incl. FAQ, pages/news/technology/case studies, geo facts) |
...
**Meta length spec** (kept within clean-SERP limits on news/products/yaml pages): `title ≤ 70` chars, `description 80–160` chars, kept in sync across locales — keeps SERP snippets clean.
...
The five solution pages are data-driven from `solution-pages.ts` (en/es/fr) and rendered through the content catch-all via the `solution-route` renderer (`src/components/marketing/solution-route.tsx`), mounted under the `solutions.tsx` layout with a hub at `solutions/index.tsx` that lists the five cards plus three program cards (`/solutions/distributors`, `/solutions/rental-operators`, `/solutions/retail-partners`) resolved from the content corpus:

| slug | URL (via `solutionPath()`) | CTA temperature |
|------|---------------------------|-----------------|
| `custom-bench-vise` | `/product-development` (`SOLUTION_PATHS` override) | hot — Request Manufacturing Proposal |
| `private-label-vises` | `/solutions/private-label-vises` | warm — Discuss Your Private Label Project |
| `industrial-vise-programs` | `/solutions/industrial-vise-programs` | warm — Request An Industrial Vise Program |
| `construction-vise-supply` | `/solutions/construction-vise-supply` | cold — Discuss Your Construction Vise Supply |
| `workshop-vise-solutions` | `/solutions/workshop-vise-solutions` | cold — Discuss Your Workshop Vise Program |

Every page shares one business logic (scenario → problems → solution → process → case study → FAQ) and ends in a **CTA temperature** (`cold` = Learn More, `warm` = Discuss Your Project, `hot` = Request Manufacturing Proposal) so each audience gets a pitch matched to how ready they are.

Legacy keyword URLs 301 to their new equivalents, preserving search equity. The mapping lives in `LEGACY_REDIRECTS` (`src/config/navigation.ts`, re-exported as `@/features/seo/legacy-redirects`) and is applied twice: the **edge gate** (`src/features/seo/edge-gate.ts`) redirects before any route runs, and `PUBLIC_PATHS` (`src/features/seo/seo.ts`) filters those keys out of the sitemap:

- SUP-era product landings (`/adventure-sup`, `/touring-sup`, …) → `/products`.
- Brand/solution landings → their bench equivalents: `/solutions/build-your-own-brand` → `/solutions/private-label-vises`; `/private-label-sup`, `/solutions/club-sup`, `/solutions/resorts-hotels`, `/solutions/paddle-clubs`, `/solutions/rental-operators`, `/solutions/retail-partners`, `/solutions/distributors` → `/solutions`.
- Marine-era `/use-cases/*` and `/solutions/*` → `/solutions` or a parent-brand page on stavalk.com.
- The solution pages are the single source of truth; `SHADOWED_PATHS` (`src/product/route-registry.ts`) covers the marketing pages so the content registry can never shadow them.

> Note: a few SUP-era paths still sit in the sitemap's `RAW_PUBLIC_PATHS` because no `LEGACY_REDIRECTS` key exists for them: `/solutions/private-label-sup`, `/solutions/resort-sup`, `/solutions/school-sup`, `/start-sup-project`, `/oem-paddle`, `/about/identity`.
...
| Images | `pnpm upload:stavalk-images` —backfills missing images to R2 (`--missing` only PUTs absent objects) |
| CI | `ci.yml` (lint + typecheck + test + build, no secrets); `deploy.yml` (push to `main`: gen `wrangler.jsonc` → build with `CLOUDFLARE_ENV=production` → **D1 migrations** → `wrangler deploy` → **purge CDN cache** → **warm edge cache** (`/`, `/es`, `/fr`, products) → `wrangler secret bulk` from GitHub secrets → **backfill missing images**). Missing `CLOUDFLARE_API_TOKEN` — whole deploy job skips gracefully. |
...
| Layer | Piece |
|-------|-------|
| **matchFaq** | Token-overlap scoring against `getSiteFaqs(locale)`: splits question + FAQ question into word tokens, computes Jaccard-like overlap (filtering to significant words, ≥ 5% overlap for English; CJK: character-level tokenization + cross-language keyword expansion, min score 0.30 vs 0.55). Returns the best-matching FAQ answer. |
...
This mode works from day one —no index build, no AI quota, no paid plan. It searches both FAQ entries AND the full content corpus (solutions, products, guides, knowledge hub, pages) using token-overlap scoring.
...
| Layer | Piece |
|-------|-------|
| Corpus | `corpus.ts` `buildChunks(locale)` —one atomic chunk per piece of info: solution pages (+ their FAQ blocks individually), knowledge hub per-section, projects, product series, guides, products/news/technology/case-studies, pages (SEO description), and every site FAQ as its own Q/A chunk; en/es/fr, URLs localized via `localizePath` |
| Embeddings | Workers AI `@cf/baai/bge-m3` (1024-dim, multilingual) in batches of 64 |
| Index | Vectorize `Stavalk-knowledge` / `-staging` / `-prod` (per env); chunk ids are stable FNV-1a hashes of `(locale, url, part)` so daily re-runs upsert in place |
