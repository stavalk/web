...
test('sitemap lists both locales of public pages with hreflang', () => {
  ...
  expect(xml).toContain(`<loc>${origin}/es/faq</loc>`)
  ...
  // sheet pages are emitted by the sitemap routes from getContentPages, not here
  expect(xml).not.toContain(`<loc>${origin}/product-development</loc>`)
  expect(xml).not.toContain(`<loc>${origin}/es/product-development</loc>`)
  expect(xml).not.toContain(`<loc>${origin}/oem-manufacturing</loc>`)
  expect(xml).not.toContain(`<loc>${origin}/solutions/school-sup</loc>`)
  expect(xml).not.toContain(`<loc>${origin}/es/solutions/school-sup</loc>`)
  expect(xml).not.toContain(`<loc>${origin}/solutions/private-label-sup</loc>`)
  expect(xml).not.toContain(`<loc>${origin}/start-sup-project</loc>`)
