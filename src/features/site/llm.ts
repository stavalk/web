...
export function llmFrenchIndex(origin: string): string {
  ...
  const projectLines = projects.fr.map((p) => `- [${p.metaTitle}](${fr(`/projects/${p.slug}`)}): ${flat(p.metaDescription)}`)
  const knowledgeLines = knowledge.fr.map((a) => `- [${a.metaTitle}](${fr(`/knowledge/${a.slug}`)}): ${flat(a.metaDescription)}`)
  const solutionLines = solutionPages.fr.map((p) => `- [${p.metaTitle}](${fr(solutionPath(p.slug))}): ${flat(p.metaDescription)}`)
  ...
  return [
    ...
    ...solutionLines,
    '',
    '### Français: Classeur de connaissances',
    ...knowledgeLines,
    '',
    '### Français: Projets',
    ...projectLines,
...
export function llmsAfarerFrenchFull(): string {
  ...
  const solutionBlocks = solutionPages.fr.map((p) =>
    [
      `# ${p.h1}`,
      '',
      ...p.intro.map(flat),
      '',
      `Scénario: ${flat(p.scenario.body)}`,
      '',
      '## Problèmes et solutions',
      ...p.pairs.flatMap((pair) => [`- Problème: ${flat(pair.problem)}`, `- Solution: ${flat(pair.solution)}`]),
      '',
      '## Processus',
      ...p.steps.map((s) => `- ${s.title}: ${flat(s.body)}`),
      '',
  ...
  const knowledgeBlocks = knowledge.fr.map((a) =>
    [`# ${a.h1}`, '', flat(a.intro), ...a.sections.flatMap((s) => ['', `## ${s.title}`, '', ...s.body.map(flat)])].join('\n'),
  )
  const projectBlocks = projects.fr.map((p) =>
    [
      `# ${p.h1}`,
      '',
      ...p.intro.map(flat),
      ...
      '',
      `Exigence: ${flat(p.requirement)}`,
      '',
      `Défi: ${flat(p.challenge)}`,
      '',
      `Solution: ${flat(p.solution)}`,
      '',
      `Produit: ${flat(p.product)}`,
      '',
      '## Processus',
      ...p.process.map((s) => `- ${s.title}: ${flat(s.body)}`),
      '',
      ...
      flat(p.result),
      '',
      `Résultat final: ${flat(p.outcome)}`,
      '',
      `Tags: ${p.tags.join(', ')}`,
    ].join('\n'),
  )
  const newsBlocks = getNewsPosts('fr').map((p) =>
    [`## Nouvelle: ${p.title}`, '', p.date.slice(0, 10), flat(p.excerpt ?? ''), '', ...mdBody(p.body)].join('\n'),
  )
  const techBlocks = getTechArticles('fr').map((a) =>
    [`## Technologie: ${a.title}`, '', flat(a.summary ?? ''), '', ...mdBody(a.body)].join('\n'),
  )
  const caseBlocks = getCaseUses('fr').map((c) => [`## Cas d'étude: ${c.title}`, '', flat(c.summary ?? ''), ...mdBody(c.body)].join('\n'))
