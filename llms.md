...
|----------|--------|---------------|-------------|
| `/llms.txt` | `src/features/site/llm.ts` + `src/features/docs/llm.ts` | Markdown index | Products + solution pages + index (factory, technology, research, news, product pages, geo facts) |
| `/llms-full.txt` | Same as `/llms.txt`, concatenated plain Markdown | Full corpus | Catalog, solutions incl. FAQ, pages/news/technology/case studies, geo facts |
...
| `/rss.xml` | news posts (RSS feed) | XML | Latest news/posts from the corpus |
...
| `/sitemap.xml` | `src/features/seo/seo.ts` (`PUBLIC_PATHS` × locales + registry) | XML | en/es/fr entries with hreflang alternates |
...
├─ solution-pages.ts — the 5 solution pages (scenario → problems → solution → FAQ, CTA temperature)
...
export function buildLlmIndex(locale: 'en' | 'es' = 'en') {
  ...
  // 4) Full content corpus (products/news/technology/case-studies/geo)
...
- **Chunk ids** are stable FNV-1a hashes of `(locale, url, part)` — daily re-runs upsert in place in Vectorize `Stavalk-knowledge` / `-staging` / `-prod`. These indexes only exist when the `vectorize` block is uncommented in `wrangler.jsonc`.
...
## 5. Meta Length Spec (kept within clean-SERP limits)

- `title ≤ 70` chars — clean SERP snippet
- `description 80–160` chars — kept in sync across locales
