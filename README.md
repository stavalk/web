...
| Page | What it does |
|------|--------------|
| **Home** | Ocean hero ("Turn your bench vise ideas into reality"), a 6-item capability strip (OEM & ODM · Private Label · Sample Service · Design & Artwork · QC on every run · Worldwide export), who-we-serve scene cards, what-we-solve, a 6-step product development process, solutions pillars, why-us cards, the **10 manufacturing platforms** table, a horizontal snap-scroll **Custom Vise Studio** walkthrough (jaw width → bolt-down base → body casting → branding → packaging)
...
| **SEO** | Per-locale sitemap with `hreflang` + canonical for the trilingual pages (en/es/fr �?marketing routes and content pages both emit alternates); OpenGraph tags (featured image is a real product photo from the site's R2 CDN), `robots.txt`, `noindex` on authenticated pages, and the 5 solution pages as keyword targets (legacy landing URLs 301 to them). Page meta is length-validated (`title �?70`, `description 80�?70`). |
