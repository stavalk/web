<div align="center">
  <h1>Stavalk</h1>
  <p>Your custom bench vise product development & manufacturing partner — 5 product series, real OEM/ODM, trilingual (en/es/fr) marketing site + 5-page solutions system, shipped edge-native on Cloudflare Workers.</p>
  <p>
  ...
  <p>
    <em>Bright Ocean Studio × Stavalk Manufacturing (Qingdao, China) — a complete custom bench vise manufacturing platform — jaw width, construction, finish, branding and packaging all adapt to your client’s brand (samples in 7-14 days, MOQ from 20 pcs).</em>
...
| `dictionary/en-product.ts` | Product-specific strings (legal, inquiry, product-specific) | Your product text |
...
Replace all `stavalk` prefixes with your `SITE_ID`:
- `name`: `myproduct`
...
This guide maps the **major architectural layers** of Stavalk to their source files, so you can jump straight to the code that matters.

| Architectural Layer | Core Concern | Primary File(s) | What It Controls |
|---------------------|--------------|-----------------|------------------|
| **Media & CDN** | Videos, PDFs, product/quality images | `public/assets/*` (git-ignored), `scripts/upload-site-assets.mjs` → R2 `stavalk-files-prod`, CDN `assets.stavalk.com/site/*` | All large binary assets; `.gitignore` prevents Git bloat; R2 key prefix controllable via `--prefix` |
| **Site Config (Framework Abstract)** | Business facts & hero content as config | Product layer `src/product/site-config.ts` exports `SITE_FACTS` (mapped from `facts.ts`) and `HERO_CONTENT` (`hero` from `content.ts`), re-exported read-only via `src/features/site/site-config.ts` as `siteConfig.facts / siteConfig.hero` | Configurable constant layer for derived sites; legacy `FACTS` / `hero` imports keep working unchanged |
...
| **AI Sales Assistant** | FAQ+corpus keyword search (free tier) / RAG Q&A (paid) | `src/features/ai/corpus.ts`, `src/features/ai/ai-chat.tsx`, Vectorize `stavalk-knowledge` | **Deploys on Workers free tier** with keyword-based search (matchFaq + matchCorpus); upgrade to Workers Paid ($5/month) for AI-powered RAG (embeddings + LLM generation). Chat shows "FAQ" or "AI" badge per answer. |
...
Stavalk is the production-ready web presence for a custom bench vise OEM/ODM manufacturer — the marketing site is positioned as a **custom bench vise product development & manufacturing partner**, not a "launch your own brand" tool. It pairs a fully designed trilingual (en / es / fr) marketing site with the complete SaaS backend from [Vectoflare](https://github.com/vectoflare/vectoflare): auth, email, an admin console, and more — every feature a real implementation, no mocks or stubs, running on the Cloudflare free-to-cheap stack (Workers + D1 + KV + R2). The full brand content (factory, technology, research, news, product pages) is served from the same Worker in en/es/fr under `/` (`/es` and `/fr` prefixed).

## The marketing site

Custom-built "Bright Ocean Studio" design language: Ocean White / Ocean Blue / Aqua / Sunset / Deep Navy palette, Manrope (display) + Inter (body) + JetBrains Mono (mono) typefaces, wave motion, glass cards, and reveal-on-scroll — light & dark modes.

| Page | What it does |
|------|--------------|
| **Home** | Qingdao-factory hero ("Custom Bench Vise Manufacturing for Brands, Distributors & Industrial Buyers"), a 6-item capability strip (OEM & ODM · Private Label · Sample Service · Design & Finish · QC on every run · Worldwide export), who-we-serve scene cards, what-we-solve, a 6-step product development process, solutions pillars, why-us cards, the **Base Platforms** grid (Light / Medium / Heavy / Pipe & Specialty covering the 10 product models), and a 5-step **Custom Vise Studio** (jaw width & opening → construction → finish & branding → jaw inserts → accessories & packaging)
| **Products** | Full catalog of the **5 real series** (SKUs, specs, prices, photos, artwork & construction lines) with SKU + price badges — see the table below |
| **Vise Design Studio** | Interactive configurator: pick colors and preview a live vise mockup — the "place your logo" pitch for prospects
| **Solutions system** | A 5-page data-driven system: custom bench vise at `/product-development` plus `/solutions/private-label-vises`, `/solutions/industrial-vise-programs`, `/solutions/construction-vise-supply`, `/solutions/workshop-vise-solutions` (paths via `solutionPath()`). Every page follows one business logic (scenario → problems → solution → process → case study → FAQ) and ends in a **CTA temperature** — cold (Learn More: construction, workshop), warm (Discuss Your Project: private label, industrial), hot (Request Manufacturing Proposal: custom bench vise) |
| **Who we serve** | Landing-oriented scene pages that funnel into the matching solution page |
| **Site search** | Two surfaces: a header search dialog fed by `/search-index.json` (the full public index, edge-cached 1h) and a `/search` page with a full-text (Orama) index built from the same data — solutions, knowledge hub, projects, product series, pages, news, technology, case studies, guides, FAQs and the six hub/landing pages (home, /products, /solutions, /projects, /knowledge, /gallery) in en/es/fr |
...
| **Legacy SEO landings** | Old keyword URLs 301 to their new equivalents — SUP-era product landings (`/adventure-sup`, `/touring-sup`, …) → `/products`; `LEGACY_REDIRECTS` (`src/config/navigation.ts`) maps `/solutions/build-your-own-brand` → `/solutions/private-label-vises`, `/private-label-sup`, `/solutions/club-sup`, `/solutions/resorts-hotels`, `/solutions/paddle-clubs`, `/solutions/rental-operators`, `/solutions/retail-partners`, `/solutions/distributors` → `/solutions`, and marine-era `/use-cases/*` + `/solutions/*` → `/solutions` or a parent-brand page on stavalk.com — search equity preserved, one source of truth |
...
| **Brand corpus** | The full ported manufacturer site (en/es/fr): `/factory/*`, `/technology/*`, `/research/*`, `/news/*`, `/products/*`, `/oem-odm-manufacturer`, `/guides`, `/faq` and more — served by a catch-all route from the bundled content |
...
**The 5 product series** (data in `src/product/series-pages.ts`, photos served from the site’s own R2 CDN)

| Series | Jaw width | Clamping force | Position |
|--------|-----------|----------------|----------|
| Light-Duty | 4″-5″ | 1,200-1,500 lbs | Compact vises for hobbyists, DIY and light metalwork |
| Medium-Duty | 6″-8″ | 3,000-4,000 lbs | Workshop workhorse for auto repair and general fabrication |
| Heavy-Duty | 8″-10″ | 6,000-8,000 lbs | Forged-steel industrial vises for mining, shipyards and heavy fabrication |
| Pipe Vises | 1/4″-6″ capacity | Chain or tripod | Plumbing, HVAC, oil & gas and fire protection |
| Specialty | 0.001″ parallelism | Precision/machine | Cross-slide, milling, drill press and CNC workholding |

Every series is a manufacturing platform — jaw width, construction, finish, branding and packaging all adapt to your client's brand; series pages quote samples in 7-14 days, bulk production in 25-35 days and MOQs from 20 pcs (standard models) up to 200+ pcs (custom tooling).

**Technical parameters** (canonical values from `src/product/facts.ts`; series/solution landing copy may state slightly different figures):

| Parameter | Value |
|-----------|-------|
| Sample lead time | 7–12 days (landing-copy variants: 7–14 days) |
| Production lead time | 20–30 days after confirmed PO + deposit (landing-copy variants: 25–35 days); custom tooling adds 15–20 days |
| MOQ | 5–10 pcs co-brand on an existing model · 20–50 pcs pilot batch · 90–100+ pcs standard run per configuration; each SKU has its own MOQ |
| Materials | HT200 / HT250 gray iron bodies; forged 45# carbon steel (heavy-duty); hardened jaw faces 58–62 HRC; specialty jaws GCr15 / Cr12MoV |
| Jaw width / force | Per the series table above — 4″–5″ @ 1,200–1,500 lbs up to 8″–10″ @ 6,000–8,000 lbs |
| Machining accuracy | 0.05 mm CNC; specialty parallelism 0.001″ (0.025 mm); CMM inspection on specialty |
| QC & testing | 7-stage gates (Nodes 01–07) · ISO 2859-1 Level II sampling · 10,000+ open/close cycle tests · salt-spray 48 h · 100-point assembly checklist · 10-year ERP traceability · optional SGS / TÜV / BV / Intertek |
| Certifications | ISO 9001, CE, RoHS (certificate numbers and validity per project on request) |
| Facility | 8,000 m² plant · 200+ workers · 3 specialized workshops · 4 automated lines · 7,000 units/month (80,000+ vises/yr capacity) |
| NDA | Issued within 4 business hours |
| Export | 40+ markets (facts.ts; marketing copy states 50+) |

**AI-ready content**: `/llms.txt` and `/llms-full.txt` index the docs, the full product catalog (names, SKUs, specs, prices, recommended use), the 5 solution pages with their FAQ, and the whole ported brand corpus (factory, technology, research, news) — so answer engines can cite the actual offering. `/entity.json` exposes the schema.org Organization entity behind the factory, and `/rss.xml` the news feed. Every entry carries a **page-level meta spec** kept within clean-SERP limits: `title ≤ 70` chars, `description 80–160` chars (kept in sync, en/es/fr).
...
| **SEO** | Per-locale sitemap with `hreflang` + canonical for the trilingual pages (en/es/fr — marketing routes and content pages both emit alternates); OpenGraph tags (featured image is a real product photo from the site's R2 CDN), `robots.txt`, `noindex` on authenticated pages, and the 5 solution pages as keyword targets (legacy landing URLs 301 to them). Page meta is kept within clean-SERP limits (`title ≤ 70`, `description 80–160` chars). |
| **AI-ready** | **Runtime:** [`llms.txt`](/llms.txt) index and [`llms-full.txt`](/llms-full.txt) full corpus —docs **plus the product catalog, the 5 solution pages (incl. FAQ) and the brand corpus**; [`entity.json`](/entity.json) schema.org Organization; [`rss.xml`](/rss.xml) news feed; `robots.txt` pointing to all of them. **Codebase:** [`AGENTS.md`](AGENTS.md) is the single source of truth for coding agents (auto-imported into [`CLAUDE.md`](CLAUDE.md)). |
...
pnpm upload:images   # backfill missing product images to R2
...
products/      Product catalog + $series detail
      solutions/     Solution hub /solutions (index maps the 5 solutions via solutionPath() + 3 program cards)
...
> R2 (object storage) is enabled by default in `wrangler.jsonc` and wired into the code (avatar upload reference). Before deploying, create the bucket: `wrangler r2 bucket create Stavalk-files` (see [storage](src/content/docs/features/storage.mdx)).
...
| `deploy.yml` | push to `main` | generates `wrangler.jsonc` from repo variables, builds with `CLOUDFLARE_ENV=production`, applies D1 migrations, deploys the Worker, **purges the CDN cache**, **warms the edge cache** (`/`, `/es`, product pages), bulk-syncs GitHub secrets —Worker secrets, and backfills missing product images to R2 |
| `upload-stavalk-images.yml` | manual | uploads the bundled images to R2 (used for one-off backfills) |
...
|----------|----------|---------------|
| `CF_PROD_D1_ID` | Yes | Production D1 database id —Cloudflare Dashboard —Workers & Pages —D1 —open database `Stavalk-db-prod` —copy `Database ID`; or `wrangler d1 list` (needs the token's D1 read permission). The deploy **fails** without it |
...
| `CF_PROD_DOMAIN` | No | Custom domain, e.g. `Stavalk.com`. When set, the deploy binds it as a custom-domain route on the production Worker (also makes the CDN purge + warm steps meaningful) |
...
| `BETTER_AUTH_URL` | Production URL, e.g. `https://Stavalk.com` (also drives canonical/sitemap origin) | **Required** |
| `RESEND_API_KEY` | [resend.com](https://resend.com) —API Keys —create a key | Emails captured to console |
| `EMAIL_FROM` | e.g. `Stavalk <noreply@Stavalk.com>` —**verify your domain in Resend first** (Settings —Domains) | No sender |
...
Create it at [dash.cloudflare.com/profile/api-tokens](https://dash.cloudflare.com/profile/api-tokens) —**Create Token** —**Create Custom Token**. Give it a name (e.g. `Stavalk-ci`), set the lifetime (≤ 1 year recommended), and tick **both** the Account resources and the Zone resources below. Then create the token **once** and copy it into the `CLOUDFLARE_API_TOKEN` GitHub secret —it is shown only a single time.
...
| D1 | Edit | `d1 migrations apply Stavalk-db-prod --env production --remote` before every deploy; `wrangler d1 list` | Yes |
| R2 Storage | Edit | creates buckets + uploads images (`scripts/upload-stavalk-images.mjs`, used by deploy backfill and the manual upload workflow) | Yes |
| Vectorize | Edit | idempotently creates the 3 knowledge indexes (`Stavalk-knowledge`, `-staging`, `-prod`) before deploy —deploy fails with code 10159 if the bound index is missing | Yes |
| Account Settings | Read | wrangler account/plan diagnostics | Recommended |

**Zone resources** (scope to the zone `Stavalk.com`):

| Resource | Permission | Why the pipeline needs it | Required |
|----------|-----------|---------------------------|----------|
| Zone | Read | `GET /zones?name=Stavalk.com` —resolves the zone id that the purge and R2-custom-domain steps target | Yes (with a custom domain) |
